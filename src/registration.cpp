...
namespace libfreenect2
{
	...
	void resize(T* src, T* dst, int src_width, int src_height, int dst_width, int dst_height)
	{
		...
		for (int x = 0; x < dst_width; x++)
		{
			...
			x_ofs[x] = sx>src_width - 1 ? src_width - 1:sx;
		}
		...
		for (int y = 0; y <dst_height; y++)
		{
			...
			int sy = (std::floor(y*scale_y))>(src_height - 1) ? (src_height - 1):(std::floor(y*scale_y));
...
public:
  ...
  void apply512_424(const Frame* rgb, const Frame* depth, Frame* undistorted, Frame* registered, const bool enable_filter, Frame* bigdepth, int* color_depth_map) const;
  void apply512_363(const Frame* rgb, const Frame* depth, Frame* undistorted, Frame* registered, const bool enable_filter, Frame* bigdepth, int* color_depth_map) const;
...
void Registration::apply512_424(const Frame *rgb, const Frame *depth, Frame *undistorted, Frame *registered, const bool enable_filter, Frame *bigdepth, int *color_depth_map) const
{
	impl_->apply512_424(rgb, depth, undistorted, registered, enable_filter, bigdepth, color_depth_map);
}
void Registration::apply512_363(const Frame *rgb, const Frame *depth, Frame *undistorted, Frame *registered, const bool enable_filter, Frame *bigdepth, int *color_depth_map) const
{
	impl_->apply512_363(rgb, depth, undistorted, registered, enable_filter, bigdepth, color_depth_map);
}
void Registration::apply640_480(const Frame *rgb, const Frame *depth, Frame *undistorted, Frame *registered, const bool enable_filter, Frame *bigdepth, int *color_depth_map) const
{
	std::shared_ptr<libfreenect2::Frame> subundistorted = std::make_shared<libfreenect2::Frame>(libfreenect2::Frame(512, 363, 4));
	std::shared_ptr<libfreenect2::Frame> subregistered = std::make_shared<libfreenect2::Frame>(libfreenect2::Frame(512, 363, 4));
	impl_->apply512_363(rgb, depth, subundistorted.get(), subregistered.get(), enable_filter, bigdepth, color_depth_map);
	resize<libfreenect2::Frame>(subundistorted.get(), undistorted, 512, 363, 640, 480);
	resize<libfreenect2::Frame>(subregistered.get(), registered, 512, 363, 640, 480);
}
...
void RegistrationImpl::apply512_424(const Frame *rgb, const Frame *depth, Frame *undistorted, Frame *registered, const bool enable_filter, Frame *bigdepth, int *color_depth_map) const
{
	// Check if all frames are valid and have the correct size
	if (!rgb || !depth || !undistorted || !registered ||
		rgb->width != 1920 || rgb->height != 1080 || rgb->bytes_per_pixel != 4 ||
...
void RegistrationImpl::apply512_363(const Frame *rgb, const Frame *depth, Frame *undistorted, Frame *registered, const bool enable_filter, Frame *bigdepth, int *color_depth_map) const
{
	// Check if all frames are valid and have the correct size
	if (!rgb || !depth || !undistorted || !registered ||
		rgb->width != 1920 || rgb->height != 1080 || rgb->bytes_per_pixel != 4 ||
		depth->width != 512 || depth->height != 424 || depth->bytes_per_pixel != 4 ||
		undistorted->width != 512 || undistorted->height != 363 || undistorted->bytes_per_pixel != 4 ||
		registered->width != 512 || registered->height != 363 || registered->bytes_per_pixel != 4)
		return;
